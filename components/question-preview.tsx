                ...
                <div className={`${viewMode === "mobile" ? "p-4 px-8" : ""} mt-auto flex-shrink-0`}>
                  ...
                  >
                    {!buttonActive && question.config.buttonTimer
                      ? `Aguarde ${question.config.buttonTimer}s...`
  ...
  return (
    <div className="space-y-4">
      {question.inputs.length === 0 && (
        <div className="flex flex-col gap-4">
          <QuestionInput icon={<User className="w-5 h-5 text-[#667085]" />} onAdd={() => onAddInput("text")} />
          <QuestionInput icon={<Phone className="w-5 h-5 text-[#667085]" />} onAdd={() => onAddInput("phone")} />
          <QuestionInput icon={<AtSign className="w-5 h-5 text-[#667085]" />} onAdd={() => onAddInput("email")} />
        </div>
      )}
      ...
      {question.inputs.length > 0 && (
        <div className="flex gap-2">
          <Button
            ...
            className="border-[#d0d5dd] text-[#667085]"
          >
...
function DraggableInput({
  ...
  input: { id: string; type: string; placeholder: string; mask?: string; iconType?: string }
  index: number
  questionId: string
  onRemove: () => void
  ...
  const { updateQuestion, updateInputMask, updateInputIconType } = useQuizStore()
  ...
  const [, drop] = useDrop(() => ({
    ...
    hover: (item: { id: string; index: number }, monitor) => {
      ...
      onReorder(questionId, item.index, index)
      item.index = index
    },
  }))
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (question) {
      const updatedInputs = question.inputs.map((i) => (i.id === input.id ? { ...i, placeholder: e.target.value } : i))
  ...
  // Função para lidar com a entrada com máscara
  const handleMaskedInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { value } = e.target
    const mask = input.mask || ""

    if (mask) {
      const maskedValue = applyMask(value.replace(/\D/g, ""), mask)
  ...
  const handleIconTypeChange = (iconType: string) => {
    updateInputIconType(questionId, input.id, iconType)
    setShowIconSelector(false)
  }

  return (
    <div ref={(node) => drag(drop(node))} className={`flex items-center gap-3 ${isDragging ? "opacity-50" : ""}`}>
      <GripVertical className="w-5 h-5 text-[#d0d5dd] cursor-grab" />

      {showIcon && (
        <div className="relative">
          <div className="flex items-center cursor-pointer" onClick={() => setShowIconSelector(!showIconSelector)}>
            {input.iconType === "text" && <User className="w-5 h-5 text-[#667085]" />}
            {input.iconType === "phone" && <Phone className="w-5 h-5 text-[#667085]" />}
            {input.iconType === "email" && <AtSign className="w-5 h-5 text-[#667085]" />}
            <svg className="w-4 h-4 ml-1 text-[#667085]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
            </svg>
          </div>

          {showIconSelector && (
            <IconSelector onSelect={handleIconTypeChange} currentType={input.iconType || input.type} />
          )}
        </div>
      )}
      ...
      <Input
        ...
        onChange={input.mask ? handleMaskedInput : handleInputChange}
...
function QuestionInput({ icon, onAdd }: { icon: React.ReactNode; onAdd: () => void }) {
  return (
    <div
      className="flex items-center gap-3 p-4 border border-[#d0d5dd] rounded-md cursor-pointer hover:bg-[#f9fafb] transition-colors"
      onClick={onAdd}
    >
      {icon}
      <span className="text-[#667085]">Insira sua pergunta aqui</span>
    </div>
  )
}
